...
It also wrote `playtest/site/index.html`, a replay website generated from
those same sessions. Confirm the site and `data.json` exist and contain every
game in the run before reviewing. The website is evidence navigation, not new
evidence: when it disagrees with a session file, the session file wins and the
site defect must be reported before the gate advances.
...
- If the run is missing — no `playtest.json`, no `run_*.json`, or no generated
  `playtest/site/index.html` and `data.json` — say so and
